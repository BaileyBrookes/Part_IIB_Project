...
#include <EEPROM.h> // To store the pressure data before logging to SD card
#include<SD.h>      // For logging
#include<SPI.h>
...
{
  readGPS();
  mySensorData = SD.open("NMEA4.txt", FILE_WRITE);
  ...
  {
    Serial.println("Readings from EEPROM");
    addr = 0;
    ...
    while(addr != EEPROM.length())
    {
        ...
        EEPROM.get(addr, altitude);
        //Serial.println(altitude);
        addr += sizeof(float);
    }
    mySensorData.close();
    //Serial.println("Readings end");
    addr = 0;
  }
  //delay(1000);
}
